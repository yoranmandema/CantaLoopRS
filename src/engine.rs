...
use crate::{
    bytecode::{ByteCodeEmitter, OpCode}, parser::parse_program, semantic_analyser::{FunctionSignature, HirBuilder, ValueKind},
    vm::VM,
};
...
impl Engine {
    ...
    pub fn get_constant(&self, id: u32, heap: &mut crate::vm::ValueHeap) -> crate::vm::Value {
        ...
        match &c.kind {
            ...
            ValueKind::Function(_) => panic!("Constant should not have Function kind"),
            ValueKind::Thunk(_) => panic!("Constant should not have Thunk kind"),
            ValueKind::Void => panic!("Constant should not have Void kind"),
        }
    }
    ...
    /// Prints timing information for each phase.
    pub fn run(&mut self, file_path: &str) {
        ...
        let hir_ast = match self.hir_builder.build(res) {
            ...
            Err(e) => {
                match e {
                    crate::semantic_analyser::HirError::TypeError(msg) => {
                    ...
                    crate::semantic_analyser::HirError::TypeMismatch { variable, expected, actual } => {
                        let expected_str = match expected {
                            ...
                            ValueKind::Function(ref ty) => ty.as_str(),
                            ValueKind::Thunk(ref ty) => ty.as_str(),
                            ValueKind::Void => "Void",
                        };
                    ...
                    crate::semantic_analyser::HirError::BinaryOpTypeError { operator, lhs_type, rhs_type, expected } => {
                        ...
                        panic!("Binary operation type error: Operator '{}' expects {}, but got {} and {}", operator, expected, lhs_str, rhs_str);
                    }
                }
            }
        };
        ...
        for func_id in function_ids.iter() {
            ...
            if self.functions.contains_key(&func_id) {
                continue; // This is a native function, don't emit bytecode for it
            }
