...
pub struct Symbol {
    ...
    pub kind: SymbolKind,
    pub ty: ValueKind,
    // Note: span tracking not yet implemented - will be added when AST tracks spans
    // pub span: Span,
}
...
impl CompilerState {
    /// Build a symbol table from HIR.
    fn build_symbol_table(hir: &HirAst) -> SymbolTable {
        ...
        for scope in &hir.scopes.scopes {
            for var in &scope.vars {
                ...
                // For now, we'll treat all variables the same way
                table.symbols.push(Symbol {
                    name: var.name.clone(),
    ...
    pub fn new(ast: crate::core::ast::Program, hir: HirAst, diagnostics: Vec<HirError>) -> Self {
        let symbols = Self::build_symbol_table(&hir);
        Self {
            ast,
            hir,
            diagnostics,
